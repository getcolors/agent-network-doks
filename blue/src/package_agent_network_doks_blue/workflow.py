...
def create_workflow():
    wf = workflow(start="agent-network-doks/start", wire_fn=wire_fn, next_fn=lambda _step, successors, opts: [] if opts.get("managed/already-destroyed") or failed(opts) else [(step, opts) for step in (successors or [])])
