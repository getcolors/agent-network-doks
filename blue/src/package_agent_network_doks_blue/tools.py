...
def tool_dir(opts: dict, tool: str) -> str:
    return stage_dir(opts, tool, default_profile="agent-network-doks")
...
def infrastructure_data(opts: dict) -> dict:
    return {**opts,
            ...
            "registry-repository": validate.registry_repository(opts)}


async def _do_api(opts: dict, path: str) -> dict | None:
    """GET a DigitalOcean API path as parsed JSON, or None when the call
    failed. Leniency is deliberate: preflights assert only on an answer, and a
    network blip falls through to the apply, which fails loudly on its own."""
    result = await runtime.exec(
        ["curl", "-fsS", "-H", f"Authorization: Bearer {opts.get('do-token')}",
...
async def doks_version_error(opts: dict) -> str | None:
    """Why the pinned DOKS version cannot be created, or None. DOKS retires
    ...
    doc = await _do_api(opts, "/kubernetes/options") or {}
    versions = (doc.get("options") or {}).get("versions") or []
    slugs = [v.get("slug") for v in versions if v.get("slug") is not None]
    if slugs and str(opts.get("doks-version")) not in slugs:
        return (f"doks-version {opts.get('doks-version')}"
                f" is not offered by DOKS; currently supported: {', '.join(slugs)}")
    return None


async def _account_registries(opts: dict) -> list[str] | None:
    """The account's registry names, or None when the API gave no answer. The
    multi-registry listing is tried first; accounts on the older
    single-registry API answer through /registry."""
    doc = await _do_api(opts, "/registries")
    if doc and doc.get("registries") is not None:
        return [str(r.get("name")) for r in doc["registries"]]
    doc = await _do_api(opts, "/registry")
    if doc and doc.get("registry"):
        return [str(doc["registry"].get("name"))]
    if await _do_api(opts, "/account"):
        return []
    return None


async def _subscription_tier(opts: dict) -> str | None:
    doc = await _do_api(opts, "/registry/subscription") or {}
    return ((doc.get("subscription") or {}).get("tier") or {}).get("slug")


async def _registry_repositories(opts: dict, registry: str) -> list[str] | None:
    """Repository names in `registry`, or None when unanswerable."""
    doc = await _do_api(opts, f"/registry/{registry}/repositoriesV2?page_size=200")
    repositories = doc.get("repositories") if doc else None
    if repositories is None:
        doc = await _do_api(opts, f"/registry/{registry}/repositories?page_size=200")
        repositories = doc.get("repositories") if doc else None
    if repositories is None:
        return None
    return [str(r.get("name")) for r in repositories]


async def registry_preflight_error(opts: dict) -> str | None:
    """Why the registry desired state cannot converge, or None — evaluated
    before anything is created, because DOCR registries are account-scoped,
    tier-limited, and their subscription is account-global.

    Create mode: zero registries → create with the requested tier; an already
    existing registry under this deployment's own name → idempotent
    re-converge, fine; other registries exist → another is permitted only when
    BOTH the account's tier and the requested tier are professional; every
    Starter/Basic multi-registry case fails toward adopt mode. No path mutates
    an existing account subscription.

    Adopt mode: the named registry must exist, and the profile repository is
    checked FIRST — an existing one (a partial converge) is reuse, not
    allocation, so a full registry never blocks idempotent recovery; only a
    repository that must be created counts against the tier's capacity."""
    target = validate.registry_name(opts)
    repo = validate.registry_repository(opts)
    names = await _account_registries(opts)
    if names is None:
        return None
    if validate.adopt_registry(opts):
        if target not in names:
            return f"digitalocean-registry-name {target} does not exist in this account"
        repos = await _registry_repositories(opts, target)
        if repos is not None and repo not in repos:
            tier = await _subscription_tier(opts)
            limit = {"starter": 1, "basic": 5}.get(tier or "")
            if limit is not None and len(repos) >= limit:
                return (f"registry {target} has no room for the {repo}"
                        f" repository on the {tier} tier; free a repository or upgrade the subscription")
        return None
    if names and target not in names:
        tier = await _subscription_tier(opts)
        requested = str(opts.get("digitalocean-registry-tier"))
        if not (tier == "professional" and requested == "professional"):
            return (f"the account already has a registry ({', '.join(names)})"
                    f" and the {tier or 'current'} subscription does not allow another; "
        ...
        if len(names) >= 10:
            return (f"the account already holds {len(names)}"
...
def push_dockerconfig_path(opts: dict) -> str:
    return str(Path(state_dir(opts)) / "push-dockerconfig.json")


def pull_dockerconfig_path(opts: dict) -> str:
    return str(Path(state_dir(opts)) / "pull-dockerconfig.json")


def persist_cluster_access(opts: dict, result: dict) -> None:
    """Write the kubeconfig, the registry facts and credentials, and the
    read-back cluster subnets where the converge scripts read them: private
    ...
    repo = validate.registry_repository(opts)
    adopted = validate.adopt_registry(opts)
    if name:
        write_private(registry_env_path(opts),
                      f"REGISTRY_HOST={sh_quote(host)}\n"
                      f"REGISTRY_NAME={sh_quote(name)}\n"
                      f"REGISTRY_REPO={sh_quote(repo)}\n"
                      f"REGISTRY_ADOPTED={sh_quote(str(adopted).lower())}\n")
    for k, path in (("push-dockerconfig", push_dockerconfig_path(opts)),
                    ("pull-dockerconfig", pull_dockerconfig_path(opts))):
...
def dns_json(opts: dict) -> str:
    ...
    return tofu.constructs_json([
        tofu.construct("resource", "cloudflare_dns_record", "agent_network_doks", {
            "zone_id": "${data.cloudflare_zone.zone.id}",
            "name": opts.get("agent-network-host"), "content": opts.get("lb-ip"),
            "type": "A", "proxied": False, "ttl": 60,
        }),
        tofu.construct("resource", "cloudflare_dns_record", "agent_network_doks_wildcard", {
            "zone_id": "${data.cloudflare_zone.zone.id}",
...
def deploy_data(opts: dict) -> dict:
    ...
    return {**opts,
            ...
            # compute name, so teardown and acceptance can find it in the
            # account by name.
            "compute-name": validate.compute_name(opts),
            # Two renderings of the same list: an unquoted YAML flow sequence
...
async def cleanup_step(opts: dict) -> dict:
    ...
    if opts.get("blue/event") == "delete":
        ...
        # kubeconfig.
        for dir in (Path(state_dir(opts)), Path(lego_dir(opts)),
                    Path(profile_dir(opts)) / "proofs"):
            if dir.exists():
