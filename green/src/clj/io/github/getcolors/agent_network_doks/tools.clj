...
(defn tool-dir [opts tool] (green-cli/stage-dir opts tool {:default-profile "agent-network-doks"}))
...
(defn infrastructure-data [opts]
  (assoc opts
         ...
         :registry-repository (validate/registry-repository opts)))

(defn- do-api
  "GET a DigitalOcean API path as parsed JSON, or nil when the call failed.
  Leniency is deliberate: preflights assert only on an answer, and a network
  blip falls through to the apply, which fails loudly on its own."
  [opts path]
  (let [{:keys [exit out]} (process/run
                            ["curl" "-fsS" "-H" (str "Authorization: Bearer " (:do-token opts))
                             (str "https://api.digitalocean.com/v2" path)]
                            {})]
    (when (zero? exit)
...
(defn- account-registries
  ...
  [opts]
  (or (some->> (get (do-api opts "/registries") "registries")
               (mapv #(get % "name")))
      (when-let [reg (get (do-api opts "/registry") "registry")]
        [(get reg "name")])
      (when (do-api opts "/account") [])))

(defn- subscription-tier [opts]
  (get-in (do-api opts "/registry/subscription") ["subscription" "tier" "slug"]))

(defn- registry-repositories
  "Repository names in `registry`, or nil when unanswerable."
  [opts registry]
  (some->> (or (get (do-api opts (str "/registry/" registry "/repositoriesV2?page_size=200"))
                    "repositories")
               (get (do-api opts (str "/registry/" registry "/repositories?page_size=200"))
                    "repositories"))
           (mapv #(get % "name"))))

(defn registry-preflight-error
  "Why the registry desired state cannot converge, or nil — evaluated before
  anything is created, because DOCR registries are account-scoped,
  ...
  [opts]
  (let [target (validate/registry-name opts)
        repo (validate/registry-repository opts)
        names (account-registries opts)]
    (when names
      (if (validate/adopt-registry? opts)
        (if-not (some #{target} names)
          (str "digitalocean-registry-name " target " does not exist in this account")
          (let [repos (registry-repositories opts target)]
            (when (and repos (not (some #{repo} repos)))
              (let [tier (subscription-tier opts)
                    limit (case tier "starter" 1 "basic" 5 nil)]
                (when (and limit (>= (count repos) limit))
                  (str "registry " target " has no room for the " repo
...
(defn push-dockerconfig-path [opts] (str (io/file (state-dir opts) "push-dockerconfig.json")))
(defn pull-dockerconfig-path [opts] (str (io/file (state-dir opts) "pull-dockerconfig.json")))
...
(defn persist-cluster-access!
  ...
  (let [host (str (output-value result :registry-host))
        ...
        repo (validate/registry-repository opts)
        adopted (validate/adopt-registry? opts)]
    (when (not-empty name)
      (write-private! (registry-env-path opts)
                      (str "REGISTRY_HOST=" (sh-quote host) "\n"
                           "REGISTRY_NAME=" (sh-quote name) "\n"
                           "REGISTRY_REPO=" (sh-quote repo) "\n"
                           "REGISTRY_ADOPTED=" (sh-quote (str adopted)) "\n"))))
  (doseq [[k path] [[:push-dockerconfig (push-dockerconfig-path opts)]
...
(defn acceptance-step [opts]
  (let [result (script-step opts "smoke.sh")]
    (if (or (wf/failed? result) (not= :create (:green/event opts)))
      result
      (assoc result :agent-network-doks/acceptance
