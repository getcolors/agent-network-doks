...
;; One SHA, one payload: this package is green-only. The payload is born
;; unpinned — no invented SHAs — and `bb pin` stamps or re-stamps it after a
;; clean, pushed HEAD. The site recognises exactly two forms, its unpinned
;; birth shape and its pinned shape, and the run fails loudly otherwise.
;;
;; The launcher pins only agent-network-doks; `green` comes transitively
;; from green/deps.edn.
  ...
  (cond (seq dirty) (do (binding [*out* *err*] (println "agent-network-doks working tree is dirty; commit before pinning")) (System/exit 2))
        ...
        :else (let [errors (atom [])]
                ...
                (if (seq @errors)
                  ...
                  (println "pinned 1 launcher to" (subs sha 0 7))))))
