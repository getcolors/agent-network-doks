...
export function toolDir(opts: Opts, tool: string): string {
  return stageDir(opts, tool, { defaultProfile: "agent-network-doks" });
}
...
export function infrastructureData(opts: Opts): Opts {
  return {
    ...
    "registry-repository": validate.registryRepository(opts),
  };
}

// GET a DigitalOcean API path as parsed JSON, or undefined when the call
...
async function doApi(opts: Opts, path: string): Promise<Record<string, unknown> | undefined> {
  const { exit, out } = await runtime.exec(
    ["curl", "-fsS", "-H", `Authorization: Bearer ${opts["do-token"]}`,
...
// Why the pinned DOKS version cannot be created, or undefined. DOKS retires
...
// API answer through /registry.
async function accountRegistries(opts: Opts): Promise<string[] | undefined> {
  const registries = (await doApi(opts, "/registries"))?.registries as
    { name?: string }[] | undefined;
  if (registries) return registries.map((r) => String(r.name));
  const single = (await doApi(opts, "/registry"))?.registry as
    { name?: string } | undefined;
  if (single) return [String(single.name)];
  if (await doApi(opts, "/account")) return [];
  return undefined;
}

async function subscriptionTier(opts: Opts): Promise<string | undefined> {
  const subscription = (await doApi(opts, "/registry/subscription"))?.subscription as
    { tier?: { slug?: string } } | undefined;
  return subscription?.tier?.slug;
}

// Repository names in `registry`, or undefined when unanswerable.
async function registryRepositories(opts: Opts, registry: string): Promise<string[] | undefined> {
  const repositories =
    ((await doApi(opts, `/registry/${registry}/repositoriesV2?page_size=200`))?.repositories ??
     (await doApi(opts, `/registry/${registry}/repositories?page_size=200`))?.repositories) as
    { name?: string }[] | undefined;
  return repositories?.map((r) => String(r.name));
}

// Why the registry desired state cannot converge, or undefined — evaluated
...
export async function registryPreflightError(opts: Opts): Promise<string | undefined> {
  const target = validate.registryName(opts);
  const repo = validate.registryRepository(opts);
  const names = await accountRegistries(opts);
  if (!names) return undefined;
  if (validate.adoptRegistry(opts)) {
    if (!names.includes(target)) {
      return `digitalocean-registry-name ${target} does not exist in this account`;
    }
    const repos = await registryRepositories(opts, target);
    if (repos && !repos.includes(repo)) {
      const tier = await subscriptionTier(opts);
      const limit = tier === "starter" ? 1 : tier === "basic" ? 5 : undefined;
      if (limit != null && repos.length >= limit) {
        return `registry ${target} has no room for the ${repo}` +
          ` repository on the ${tier} tier; free a repository or upgrade the subscription`;
      }
    }
    return undefined;
  }
  if (names.length > 0 && !names.includes(target)) {
    const tier = await subscriptionTier(opts);
    const requested = String(opts["digitalocean-registry-tier"]);
    if (!(tier === "professional" && requested === "professional")) {
      return `the account already has a registry (${names.join(", ")})` +
        ` and the ${tier ?? "current"} subscription does not allow another; ` +
    ...
    if (names.length >= 10) {
      return `the account already holds ${names.length}` +
...
export function pushDockerconfigPath(opts: Opts): string {
  return join(stateDir(opts), "push-dockerconfig.json");
}

export function pullDockerconfigPath(opts: Opts): string {
  return join(stateDir(opts), "pull-dockerconfig.json");
}
...
export function persistClusterAccess(opts: Opts, result: Opts): void {
  ...
  const repo = validate.registryRepository(opts);
  const adopted = validate.adoptRegistry(opts);
  if (name.length > 0) {
    writePrivate(registryEnvPath(opts),
                 `REGISTRY_HOST=${shQuote(host)}\n` +
                 `REGISTRY_NAME=${shQuote(name)}\n` +
                 `REGISTRY_REPO=${shQuote(repo)}\n` +
                 `REGISTRY_ADOPTED=${shQuote(String(adopted))}\n`);
  }
  for (const [k, path] of [["push-dockerconfig", pushDockerconfigPath(opts)],
                           ["pull-dockerconfig", pullDockerconfigPath(opts)]] as const) {
...
export async function cleanupStep(opts: Opts): Promise<Opts> {
  if (opts["red/event"] === "delete") {
    ...
    // kubeconfig.
    for (const dir of [stateDir(opts), legoDir(opts), join(profileDir(opts), "proofs")]) {
