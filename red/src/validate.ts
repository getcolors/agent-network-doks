...
export function ipv4Cidr(s: unknown): boolean {
  if (!cidrRe.test(String(s))) return false;
  const [addr, prefix] = String(s).split("/");
  const octets = String(addr).split(".").map(Number);
  return octets.every((o) => o >= 0 && o <= 255) &&
    Number(prefix) >= 0 && Number(prefix) <= 32;
}
...
const doksVersionRe = /^[0-9]+\.[0-9]+\.[0-9]+-do\.[0-9]+$/;
const modelIdRe = /^[A-Za-z0-9][A-Za-z0-9._:-]*$/;
// DigitalOcean resource names accept letters, digits, dashes and periods.
const doNameRe = /^[A-Za-z0-9][A-Za-z0-9.-]{0,62}$/;
// DOCR registry names: lowercase alphanumerics and hyphens.
const doRegistryRe = /^[a-z0-9][a-z0-9-]{1,62}$/;
export const registryTiers = ["starter", "basic", "professional"];
...
export function adoptRegistry(opts: Opts): boolean {
  return !placeholder(opts["digitalocean-registry-name"]);
}
...
export function registryName(opts: Opts): string {
  return adoptRegistry(opts)
    ? String(opts["digitalocean-registry-name"]).trim()
    : utils.registryName(computeName(opts));
}
...
export function registryRepository(opts: Opts): string {
...
export function registryErrors(opts: Opts): string[] {
  const errors: string[] = [];
  if (adoptRegistry(opts)) {
    if (!doRegistryRe.test(String(opts["digitalocean-registry-name"]).trim())) {
      errors.push(":digitalocean-registry-name must be lowercase alphanumerics and hyphens");
    }
    if (!missing(opts["digitalocean-registry-tier"])) {
      errors.push(":digitalocean-registry-tier is create-mode-only; remove it when adopting a registry via :digitalocean-registry-name");
    }
  } else {
    if (missing(opts["digitalocean-registry-tier"])) {
      errors.push(":digitalocean-registry-tier is required when no :digitalocean-registry-name adopts an existing registry");
    }
    if (!missing(opts["digitalocean-registry-tier"]) &&
        !registryTiers.includes(String(opts["digitalocean-registry-tier"]))) {
      errors.push(":digitalocean-registry-tier must be starter, basic, or professional");
    }
    if (!doRegistryRe.test(registryName(opts))) {
      errors.push(`the profile-derived registry name ${registryName(opts)} is not a valid DOCR name`);
    }
  }
  return errors;
}
...
export const providerSecrets = ["do-token", "cloudflare-api-token"];
