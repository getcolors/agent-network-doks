...
import { agentNetworkDoksWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify the Agent Network demo on DOKS\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(agentNetworkDoksWorkflow, defaultArgs(args));
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(agentNetworkDoksWorkflow, defaultArgs(args));
  }
