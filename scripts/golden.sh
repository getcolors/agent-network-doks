...
# agent-network-doks is a single colour, so there is no parity harness. This is
...
for backend in local r2; do
  ...
  deploy="$actual/agent-network-doks-deploy"
  ...
  # a literal address, digest or subnet here would mean a run-time fact was
  # templated and the goldens stopped being workstation-independent. The pod
  # CIDR is on this list because DOKS subnets are READ BACK from the cluster,
  # never desired-state input.
  for pair in "manifests/proxy.yaml:__TRAEFIK_INTERNAL_IP__" \
              "manifests/proxy.yaml:__POD_CIDR__" \
              "netbird-config.yaml:__POD_CIDR__" \
