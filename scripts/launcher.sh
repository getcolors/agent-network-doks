...
[[ -L "$root/red/red" ]] && [[ $(readlink "$root/red/red") == ../skills/package-agent-network-doks-red/red ]]
[[ -L "$root/blue/blue" ]] && [[ $(readlink "$root/blue/blue") == ../skills/package-agent-network-doks-blue/blue ]]
...
for colour in red blue; do
  payload="$root/skills/package-agent-network-doks-$colour/$colour"
...
red_launcher="$root/skills/package-agent-network-doks-red/red"
...
if grep -qE '"package-agent-network-doks-red": "github:getcolors/agent-network-doks#[0-9a-f]{40}",' "$red_launcher"; then
  mkdir "$tmp/red-cold"
  ...
  [[ -f "$tmp/red-cold/.colors/agent-network-doks-fixture/compute/managed-kubernetes/managed-kubernetes.tf.json" ]] || fail 'cold red payload rendered no compute document'
  [[ -f "$tmp/red-cold/.colors/agent-network-doks-fixture/agent-network-doks-deploy/converge.sh" ]] || fail 'cold red payload rendered no deploy stage'
fi
