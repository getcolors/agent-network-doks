...
# AGENT_NETWORK_DOKS_LIB_ROOT overrides name the repository root), while green,
...
build_variant() {
  ...
  for colour in green red blue; do
    ...
    case $colour in
      ...
      red)   (cd "$root/red" && env AGENT_NETWORK_DOKS_LIB_ROOT="$root" "$@" \
                ./red build -f "$state" >/dev/null) ;;
      blue)  (cd "$root/blue" && env "$@" \
                uv run python -m package_agent_network_doks_blue build -f "$state" >/dev/null) ;;
    esac
  done
...
echo "green, red, and blue Agent Network DOKS artifacts are byte-identical"
