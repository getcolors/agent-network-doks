...
description: Deploy, converge, inspect, or delete a NetBird Agent Network demo on DigitalOcean Kubernetes. Use for a keyless LLM gateway or an isolated AI agent on managed Kubernetes, with model allowlists, budget limits, and NetworkPolicy checks.
---

# NetBird Agent Network on managed Kubernetes

This Green package configures a NetBird gateway and a two-pod application.
...
state at `<profile>/agent-network-doks-registry.tfstate`. Use R2 or S3.
R2 requires `COLORS_PAR_R2_ACCESS_KEY_ID` and `COLORS_PAR_R2_SECRET_ACCESS_KEY`.
S3 uses the ambient AWS credential chain.

Existing combined `agent-network-doks-infrastructure.tfstate` must be explicitly
...
The package creates a DigitalOcean Container Registry or adopts one named by `digitalocean-registry-name`. Adopt mode requires omitting `digitalocean-registry-tier`. It owns only the profile repository inside an adopted registry.
