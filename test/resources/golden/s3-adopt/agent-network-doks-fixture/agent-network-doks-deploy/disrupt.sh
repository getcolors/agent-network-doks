...
log() { echo "agent-network-doks-disrupt: $*" >&2; }

regate() { bash "$DIR/smoke.sh" --post-disrupt; }

# Preflight: every node Ready and none cordoned before deliberately breaking
# things — a suite that starts during DOKS maintenance or a half-capacity
# window would blame its own disruptions for someone else's.
notready=$(kubectl get nodes --no-headers | awk '$2 != "Ready" {print $1}')
[[ -z $notready ]] || { log "FATAL: nodes not Ready before the suite: $notready"; exit 1; }
cordoned=$(kubectl get nodes -o jsonpath='{range .items[?(@.spec.unschedulable==true)]}{.metadata.name}{"\n"}{end}')
[[ -z $cordoned ]] || { log "FATAL: cordoned nodes before the suite (maintenance or upgrade in progress?): $cordoned"; exit 1; }
...
# RWO volumes move with their pods only after the CSI detach completes;
# bounded wait so a slow detach surfaces here instead of as a rollout
# timeout blamed on the application.
log "waiting for volume attachments to leave the drained node"
for _ in $(seq 1 60); do
  stuck=$(kubectl get volumeattachment -o jsonpath="{range .items[?(@.spec.nodeName=='$node')]}{.metadata.name}{'\n'}{end}" 2>/dev/null | grep -c . || true)
  [[ ${stuck:-0} -eq 0 ]] && break
  sleep 10
done
