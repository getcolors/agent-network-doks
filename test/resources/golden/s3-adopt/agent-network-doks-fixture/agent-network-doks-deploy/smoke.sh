...
KUBE_DNS_IP=$(kubectl -n kube-system get svc kube-dns -o jsonpath='{.spec.clusterIP}' 2>/dev/null || echo "10.245.0.10")
...
gate_isolation() {
  ...
  # IPv6 targets included: managed-Kubernetes plumbing may hand pods
  # link-local or unique-local IPv6 without public reachability, so no
  # route-table shape is asserted — reachability is the claim, and it is
  # probed per family.
...
# --- LB firewall vs desired http-sources -------------------------------------
#
# loadBalancerSourceRanges is enforced by the cloud controller as LB firewall
# rules, and an open (0.0.0.0/0) deployment cannot prove denial by probing —
# so the DigitalOcean API is the gate: the effective client-source allow set
...
bash "$DIR/managed-ingress.sh" "$LB_IP" 0.0.0.0/0
...
if [[ -d /dev/shm && -e "/dev/shm/agent-network-doks-$profile-setup-key" ]]; then
