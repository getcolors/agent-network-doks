...
API="https://agent-network-doks.example.com/api"
...
if [[ -d /dev/shm ]]; then KEY_FILE="/dev/shm/agent-network-doks-$PROFILE-setup-key"
else KEY_FILE="$STATE/setup-key"; fi

log() { echo "agent-network-doks-bootstrap: $*" >&2; }
...
if [[ -z $endpoint ]]; then
  log "bootstrapping the agent network against proxy cluster agent-network-doks.example.com"
  for i in $(seq 1 30); do
    out=$(curl -sS -o /tmp/.an-settings -w '%{http_code}' -X POST "$API/agent-network/settings" \
      ...
      --data "$(jq -nc --arg p "agent-network-doks.example.com" '{proxy_address:$p}')" || true)
...
if [[ $observed_proxy != "agent-network-doks.example.com" ]]; then
  log "FATAL: settings carry proxy_address $observed_proxy, not this deployment's agent-network-doks.example.com"
  exit 1
fi
