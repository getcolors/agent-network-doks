...
log() { echo "agent-network-doks-agent: $*" >&2; }

# shellcheck disable=SC1091
source "$STATE/registry.env"
: "${REGISTRY_HOST:?}" "${REGISTRY_NAME:?}" "${REGISTRY_REPO:?}"
...
image="$REGISTRY_HOST/$REGISTRY_NAME/$REGISTRY_REPO@$(cat "$STATE/agent-image-digest")"
